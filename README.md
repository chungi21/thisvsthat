# 팀 프로젝트 | This VS That(이거저거) - 투표형 게시판 플랫폼 (Spring Boot, JPA, JWT, WebSocket)
This VS That (이거저거)은 사용자가 다양한 고민을 쉽고 재미있게 해결할 수 있도록 돕는 투표형 게시판 플랫폼입니다. 올인(All-in) 또는 올아웃(All-out) 방식의 직관적인 투표 시스템을 중심으로, 게시판 CRUD, 무한 스크롤, WebSocket 기반 실시간 채팅, JWT 인증 등을 구현했습니다.<br/>

본 프로젝트는 Spring Boot, Spring Security, JPA, JWT를 활용한 첫 번째 팀 프로젝트로, 기획자나 디자이너 없이 전원이 개발자로 구성된 팀이 기획 → 디자인 → 개발 전 과정을 직접 수행했습니다.<br/>

협업 과정에서는 Notion, Figma, GitHub를 적극 활용하여 체계적인 워크플로우를 유지하려 노력했으며, GitHub 브랜치 전략과 Pull Request 리뷰 및 병합 과정을 경험하며 실제 협업 환경에 가까운 개발 프로세스를 체득할 수 있었습니다.<br/>

이번 프로젝트를 통해 단순히 기능 구현을 넘어, 팀원들과의 역할 분담, 코드 스타일 조율, 충돌 해결 경험 등 협업에서 중요한 부분들을 체험할 수 있었습니다.<br/>
