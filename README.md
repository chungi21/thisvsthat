...
* 데이터베이스 및 엔티티 관리
  * Entity 연관관계 설정 (회원–게시글–댓글 등 관계 매핑)
* 실시간 기능
...
Notion과 Figma 등을 활용해 협업하고 기획을 시각화하려고 노력했으며, 팀 내 규칙도 세우며 체계적으로 진행하고자 했지만, 실제 개발 과정에서는 일부 규칙이 잘 적용되지 않아 수정과 조율이 반복적으로 필요했습니다.

여러 명이 함께 개발하다 보니, 기획한 내용이 실제로 구현된 부분과 어떻게 다른지 확인할 수 있었고, 서로의 코드 스타일을 비교해보며 다양한 접근 방식을 배울 수 있었습니다.

혼자 개발하는 것과 달리, 팀 프로젝트에서는 GitHub를 활용한 협업 방식, 예를 들어 브랜치 전략, 코드 병합, 작업 분담 등에 대해 배우고 경험할 수 있었습니다.

이 프로젝트에 참여한 이유는 여럿이 함께할 때 실제로 어떤 방식으로 프로젝트가 진행되는지를 경험해보고, 또한 혼자서가 아니라 팀 단위로 작업할 때 GitHub를 어떻게 활용할 수 있는지를 직접 체험해보고 싶었기 때문입니다. 실제 현업 방식은 알 수 없지만, 병합 시 충돌이 발생하지 않도록 프로젝트 초반에 역할을 분담하고 설정을 진행했습니다. 예를 들어 누군가는 초기 설정을 맡고, 누군가는 퍼블리싱을 맡아 시작했으며, 디렉토리 구조 또한 Controller, Service, DTO, Entity 같은 계층별 구분이 아니라 페이지 단위와 기능 단위별로 디렉토리를 나누어 작업했습니다.

그 결과 병합 시에도 큰 문제가 없었고, 충돌이 발생하더라도 동일 파일에서 수정 내용이 반영되지 않아 생기는 단순 충돌 정도여서 쉽게 해결할 수 있었습니다. 또한 팀원들이 각자 작업을 진행할 때 GitHub에 누군가 push하면 다른 팀원들이 반드시 pull을 통해 최신 상태를 반영한 뒤 작업을 이어갔기 때문에, 협업 과정에서 큰 문제 없이 원활하게 프로젝트를 진행할 수 있었습니다.

특히 저는 기능 구현에 크게 참여하지는 않았지만, 팀 내에서 GitHub 병합 담당자 역할을 맡아 Pull Request를 확인하고 병합하는 업무를 주로 수행했습니다. 이를 통해 코드 리뷰 및 충돌 관리 과정을 경험할 수 있었고, 협업 환경에서 중요한 책임을 맡아볼 수 있었습니다.
